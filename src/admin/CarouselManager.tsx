  ...
  return (
    <div className="space-y-8">
      <div className="bg-white rounded-2xl shadow p-6 border">
        <h3 className="font-display text-xl font-bold text-primary mb-4">Add New Slide</h3>
        ...
        <button disabled={busy} onClick={addSlide} className="mt-4 px-6 py-2.5 rounded-lg bg-primary text-primary-foreground font-medium disabled:opacity-50">
      ...
      <div>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {slides.map((s, i) => (
            <div key={s.id} className="bg-white rounded-xl shadow border overflow-hidden">
              ...
              <div className="p-3 space-y-2">
                ...
                <div className="flex gap-1">
                  ...
                  <button onClick={() => setSlides(slides.map((x) => x.id === s.id ? { ...x, active: !x.active } : x))} className="p-2 rounded hover:bg-muted">
                    {s.active ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4 text-muted-foreground" />}
                  </button>
                  <button onClick={() => setSlides(slides.filter((x) => x.id !== s.id))} className="p-2 rounded hover:bg-destructive/10 text-destructive ml-auto"><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
            </div>
          ))}
