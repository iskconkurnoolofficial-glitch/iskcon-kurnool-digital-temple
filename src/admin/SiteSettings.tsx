...
export default function SiteSettingsForm() {
  ...
  const onLogo = async (f: File) => {
    try { const url = await uploadToCloudinary(f); setS({ ...s, logo: url }); } catch { alert("Upload failed"); }
  };

  const Field = ({ k, label, type = "text" }: { k: keyof typeof s; label: string; type?: string }) => (
    <label className="block">
      <span className="text-sm font-medium text-foreground/80 mb-1 block">{label}</span>
      <input type={type} className="w-full px-4 py-2.5 border rounded-lg" value={s[k] as string} onChange={(e) => setS({ ...s, [k]: e.target.value })} />
    </label>
  );
  ...
  return (
    <div className="bg-white rounded-2xl shadow p-6 border space-y-6 max-w-3xl">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <Field k="instagram" label="Instagram URL" />
        <Field k="youtube" label="YouTube Channel URL" />
      </div>

      <label className="block">
        <span className="text-sm font-medium text-foreground/80 mb-1 block">Google Maps Embed URL</span>
        <input className="w-full px-4 py-2.5 border rounded-lg" value={s.mapEmbed} onChange={(e) => setS({ ...s, mapEmbed: e.target.value })} />
      ...
      <label className="block">
        ...
        <textarea className="w-full px-4 py-2.5 border rounded-lg" rows={4} value={s.address} onChange={(e) => setS({ ...s, address: e.target.value })} />
      </label>

      <Field k="footer" label="Footer Copyright Text" />
