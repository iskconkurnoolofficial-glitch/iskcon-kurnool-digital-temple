...
import { useAdmin, defaultTerms } from "@/context/AdminContext";
...
export const Route = createFileRoute("/terms")({
  head: () => ({
    meta: [
      { title: "Terms & Conditions — ISKCON Kurnool" },
      {
        name: "description",
        content: "Official Terms & Conditions for ISKCON Kurnool digital temple services, donations, and programmes.",
      },
    ],
  }),
  component: TermsPage,
...
function TermsPage() {
  ...
  // Helper to render lines, lists, and contact details
    ...
    return (
      <div className="space-y-3 text-slate-700 text-base leading-relaxed">
        {lines.map((line, idx) => {
          ...
          // Subheadings (e.g. ### Heading or **Subheading**)
        ...
        {/* Dynamic Contact Details for Section 27 */}
        {isContactSection && (
        ...
        <div className="max-w-4xl mx-auto px-6 sm:px-8 space-y-8">
          ...
          <header className="space-y-4">
            <h1 className="font-display font-black text-3xl sm:text-5xl text-[#5b2c9b] tracking-tight">
              {currentTerms.introTitle || "Terms & Conditions"}
            </h1>

            <div className="flex items-center gap-2 text-sm text-slate-500">
              <Calendar className="h-4 w-4 text-slate-400" />
              <span><strong>Last Updated:</strong> {currentTerms.lastUpdated || "24 August 2026"}</span>
            </div>

            <p className="text-base sm:text-lg text-slate-700 leading-relaxed pt-2">
              {renderInlineFormatting(
                currentTerms.introText ||
                "Welcome to the official website of **ISKCON Kurnool / ISKCON Kurnool Temple**. By accessing or using this website, you agree to be bound by these Terms & Conditions. Please read them carefully before using the website, making a donation, registering for a programme, or submitting any information."
              )}
            </p>
          </header>
          ...
          <main className="space-y-10">
            {currentTerms.sections.map((sec) => {
              const isContactSec = sec.number === "27" || sec.title.toLowerCase().includes("contact us");
              return (
